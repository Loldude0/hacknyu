...
ACCOUNTS_TO_SCRAPE = [
    "elonmusk", "realDonaldTrump", "VitalikButerin", "cz_binance", "saylor",
    "jack", "APompliano", "RaoulGMI", "Grayscale", "BarrySilbert",
    "adam3us", "Cobratate", "novogratz", "solana", "aantonop",
    "LynAldenContact", "CathieDWood", "SatoshiLite", "IOHK_Charles", "rogerkver",
    "JoelKatz", "brian_armstrong", "jespow", "mcuban", "balajis",
    "NavalRavikant", "cdixon", "TimDraper", "danheld", "Ethereum",
    "crypto", "100trillionUSD", "TheBlock__", "CoinDesk", "coryklippsten",
    "PeterSchiff", "trondao", "jackmallers", "0xPolygonEco", "NickSzabo4",
    "BTC_Archive", "BTCTN", "CaitlinLong_", "PanteraCapital", "Delphi_Digital",
    "sama", "cobie", "StaniKulechov", "TheSpartanGroup", "GVteam"
]
...
async def analyze_tweet_with_gemini(tweet_text: str, username: str) -> Optional[TweetAnalysis]:
    try:
...
async def fetch_user_tweets(api, username: str, existing_data: Dict) -> List[Dict]:
    try:
        ...
        # Get user tweets
        tweets = []
        async for tweet in api.user_tweets(username, limit=100):
            ...
            analysis = await analyze_tweet_with_gemini(tweet.text, username)
...
async def scrape_tweets():
    ...
    while True:
        try:
            ...
            for i in range(0, len(ACCOUNTS_TO_SCRAPE), 50):
                ...
                for username in batch:
                    ...
                    if new_tweets:
                        ...
                        print(f"Fetched and analyzed {len(new_tweets)} crypto-related tweets from {username}")
            ...
            print("Completed full cycle. Waiting 16 minutes before starting next cycle...")
